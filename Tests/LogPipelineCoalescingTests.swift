...
final class LogPipelineCoalescingTests: XCTestCase {
    ...
    func testRevisionBumpsAreCoalescedUnderBurstButNoLinesAreLost() async throws {
        ...
        let prefix = "coalescing-test-\(UUID().uuidString): "
        let expected = (0..<200).map { "\(prefix)burst line \($0)" }
        for message in expected { LogStore.shared.log(.connection, message) }
        ...
        // Coalescing drops UI updates, never log lines. The shared test host
        // can log during either await, so assert exactly OUR burst, in order.
        let actual = (LogStore.shared.entries[.connection] ?? [])
            .filter { $0.text.hasPrefix(prefix) }
            .map(\.text)
        XCTAssertEqual(actual.count, 200)
        XCTAssertEqual(actual, expected, "every burst line must survive exactly once, in order")
    }
