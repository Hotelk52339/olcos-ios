...
final class LogExportTests: XCTestCase {
    ...
    func testFileNameShape() {
        ...
        XCTAssertEqual(name, "olcOS-connection-v1-3-b263-20260618-205641.log")
    }

    func testCombinedFileNameShape() {
        let name = LogExport.combinedFileName(version: "1.3", build: "263",
                                              date: fixedDate, timeZone: utc)
        XCTAssertEqual(name, "olcOS-all-logs-v1-3-b263-20260618-205641.log")
    }
    ...
    func testHeaderCarriesIdentity() {
        ...
        XCTAssertTrue(header.contains("olcOS 1.3 (build 263)"))
    ...
    func testOlcosFirstReleaseExportHasExactBrandAndResetVersion() {
        XCTAssertEqual(LogExport.fileName(label: "connection", version: "1.0", build: "1",
                                          date: fixedDate, timeZone: utc),
                       "olcOS-connection-v1-0-b1-20260618-205641.log")
        XCTAssertEqual(LogExport.combinedFileName(version: "1.0", build: "1",
                                                  date: fixedDate, timeZone: utc),
                       "olcOS-all-logs-v1-0-b1-20260618-205641.log")
        let header = LogExport.header(
            title: "Connection", fileName: "connection.log",
            version: "1.0", build: "1", device: "iPhone", os: "iOS",
            lineCount: 0, first: nil, last: nil,
            exportedAt: fixedDate, timeZone: utc)
        XCTAssertTrue(header.contains("olcOS · diagnostic log export"))
        XCTAssertTrue(header.contains("olcOS 1.0 (build 1)"))
        XCTAssertFalse(header.contains("olcrtc"))
    }
