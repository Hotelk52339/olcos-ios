...
final class UpdateCheckerTests: XCTestCase {
    ...
    // olcOS starts a new 1.0/build 1 line. Keep numeric version ordering:
    // this is intentionally NOT a forced downgrade bridge for legacy installs.
    func testOlcosFirstReleaseEqualsInstalledBuildOne() {
        XCTAssertFalse(UpdateChecker.isNewer(latestTag: "v1.0.1", thanCurrent: "1.0.1"))
        XCTAssertEqual(UpdateChecker.normalize("  V1.0.1\n"), "1.0.1")
    }

    func testOlcosNextBuildIsOfferedAndOlderBuildIsNot() {
        XCTAssertTrue(UpdateChecker.isNewer(latestTag: "v1.0.2", thanCurrent: "1.0.1"))
        XCTAssertTrue(UpdateChecker.isNewer(latestTag: "v1.0.10", thanCurrent: "1.0.9"))
        XCTAssertFalse(UpdateChecker.isNewer(latestTag: "v1.0.1", thanCurrent: "1.0.2"))
    }

    func testOlcosResetRequiresManualInstallFromLegacyLine() {
        XCTAssertFalse(UpdateChecker.isNewer(latestTag: "v1.0.1", thanCurrent: "1.3.281"))
        // A release title is not a valid version tag; tag_name remains authoritative.
        XCTAssertFalse(UpdateChecker.isNewer(latestTag: "olcOS1.0", thanCurrent: "1.0.1"))
    }
    ...
    func testIpaDownloadURLMatchesReleaseWorkflowShape() {
        ...
        XCTAssertEqual(AppConstants.Update.ipaDownloadURL(tag: "v1.4"),
                       "https://github.com/\(AppConstants.Update.repoSlug)/releases/download/v1.4/olcos-ios-unsigned.ipa")
    }

    func testOlcosReleaseUsesExactRepositoryAndAsset() {
        XCTAssertEqual(AppConstants.Update.repoSlug, "Hotelk52339/olcos-ios")
        XCTAssertEqual(AppConstants.Update.ipaAssetName, "olcos-ios-unsigned.ipa")
        XCTAssertEqual(AppConstants.Update.latestReleaseAPIURL.absoluteString,
                       "https://api.github.com/repos/Hotelk52339/olcos-ios/releases/latest")
        XCTAssertEqual(AppConstants.Update.releasePageURL(tag: "v1.0.1").absoluteString,
                       "https://github.com/Hotelk52339/olcos-ios/releases/tag/v1.0.1")
        let ipa = "https://github.com/Hotelk52339/olcos-ios/releases/download/v1.0.1/olcos-ios-unsigned.ipa"
        XCTAssertEqual(AppConstants.Update.ipaDownloadURL(tag: "v1.0.1"), ipa)
        XCTAssertEqual(AppConstants.Update.sideStoreURL(tag: "v1.0.1")?.absoluteString,
                       "sidestore://install?url=\(ipa)")
        XCTAssertEqual(AppConstants.Update.liveContainerURL(tag: "v1.0.1")?.absoluteString,
