...
//   • a descriptive FILENAME  — olcOS-connection-v1-0-b1-20260714-133041.log
...
// "# olcOS <v> build <b>" session banner in LogStore.
//
...
enum LogExport {
    ...
    /// `olcOS-<label>-v<version>-b<build>-<stamp>.log`.
    static func fileName(label: String, version: String, build: String,
                         date: Date, timeZone: TimeZone = .current) -> String {
        "olcOS-\(safeToken(label))-v\(safeToken(version))-b\(safeToken(build))-\(fileStamp(date, timeZone: timeZone)).log"
    }

    /// Filename for the combined "Export all" file.
    static func combinedFileName(version: String, build: String,
                                 date: Date, timeZone: TimeZone = .current) -> String {
        "olcOS-all-logs-v\(safeToken(version))-b\(safeToken(build))-\(fileStamp(date, timeZone: timeZone)).log"
    }
    ...
    static func header(title: String, fileName: String,
        ...
        return [
            bar,
            " olcOS · diagnostic log export",
            thin,
            row("Log", title),
            row("File", fileName),
            row("App", "olcOS \(version) (build \(build))"),
    ...
    static func exportAll(sections: [Section]) -> URL? {
        ...
        var parts: [String] = [
            bar,
            " olcOS · FULL diagnostic log export (\(nonEmpty.count) logs)",
